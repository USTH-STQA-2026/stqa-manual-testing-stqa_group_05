...
|---|---|---|---|
| Email | Exist | `librarian@library.com` | Login successfully |
...
| Book state after returning | Available | `BOOK013` | Book status returns to "Available" |
...
| TC-15 | Search by author with result | User account login successfully | 1. Login Library App<br>2. Open "Books" tab<br>3. Observe Search Bar `Search by book title or author...`<br>4. Enter keyword in Search Bar | Email: `ba.nguyen@email.com`<br>Password: `password123`<br>Keyword: `Nguyễn Minh Đức` | Display books `BOOK001`, `BOOK009` and their full info | REQ-03 | EP |
...
| TC-33 | Member cannot add new member | User logged in account is `Member` | 1. Login Library App as `Member`<br>2. Observe App Bar | Member account: `ba.nguyen@email.com`, password: `password123` | `Add member` icon is not displayed<br>Member cannot have access to add new member | REQ-07 | EP |
...
| TC-43 | Librarian can view all Borrow Records | User logged in account is `Librarian` | 1. Login Library App as `Librarian`<br>2. Open `Borrow / Return` tab | Librarian account: `librarian@library.com`, password: `admin123` | Display all borrow records: `BR001`, `BR002`, `BR003`, `BR004`, `BR005` | REQ-08 | EP |
...
| Member management | 11 | REQ-07 | EP |
| Lookup | 3 | REQ-08 | EP |
